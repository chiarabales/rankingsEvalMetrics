...
def main(_arraylength, _trials, _distance):
    ...
    _path = pathlib.Path.home() / '21_comparing_rankings/results/robustness/Robustness1A/'
    _path.mkdir(exist_ok=True)
    _file_name = f'ROB1a_2_arraylength_{_arraylength}_trials_{_trials}_distance_{_distance}.txt'
    ...
    count_neg = [0 for i in range(len(metrics))]
    ...
    for t in range(_trials):
        ...
        for i in range(0, len(S)-1):
            for j in range(i+1, len(S)-1):
                ...
                S_tilde = S
                S_tilde = _utils.swapElements(S_tilde, i, j) # swap
                ...
                for _relevant in range(1,int(_arraylength/2)+1):
                    ...
                    for met in metrics:
                        ...
                        if met not in metrics_withNoRelevance:
                            ...
                            n = met(S, R, _relevant)
                            ntilde = met(S_tilde, R, _relevant)
